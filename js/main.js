...
async function sendimg() {
  ...
  if (appVersion < 0x16) {
    if (mode.startsWith('bwr')) {
      await epdWrite(driver === "01"|"02"|"08" ? 0x24 : 0x10, canvas2bytes(canvas, 'bw'));
      await epdWrite(driver === "01"|"02"|"08" ? 0x26 : 0x13, canvas2bytes(canvas, 'red', driver === '02'));
    } else {
...
function handleNotify(value, idx) {
  ...
  if (idx == 0) {
    ...
    // 更新驱动显示和画布尺寸
    if (driverSizeMap[driverCode]) {
      // 确保UI中的驱动选择器显示正确的驱动编号
      epddriver.value = driverCode;
      
      const {width, height} = driverSizeMap[driverCode];
      canvas.width = width;
...
document.body.onload = () => {
  ...
  // 强制设置画布尺寸为默认驱动对应的尺寸
  const driverSelect = document.getElementById('epddriver');
  const defaultOption = driverSelect.options[driverSelect.selectedIndex];
  canvas.width = parseInt(defaultOption.getAttribute('data-width'));
  canvas.height = parseInt(defaultOption.getAttribute('data-height'));
